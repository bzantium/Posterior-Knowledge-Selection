...
class Manager(nn.Module):
    ...
    def forward(self, x, y, K):
        ...
        k_logits = F.log_softmax(self.mlp_k(k_i), dim=-1)
...
class Attention(nn.Module):
    ...
    def score(self, hidden, encoder_outputs):
        ...
        attn_scores = F.tanh(self.attn(torch.cat((hidden, encoder_outputs), dim=-1)))  # [n_batch, seq_len, n_hidden]
...
class Decoder(nn.Module):
    ...
    def forward(self, input, k, hidden, encoder_outputs):
        ...
        t_hidden = F.tanh(torch.cat((self.y_weight(y_hidden), self.k_weight(k_hidden)), dim=-1))
        # t_hidden: [n_layer, n_batch, 2*n_hidden]
        r = F.sigmoid(self.z_weight(t_hidden))  # [n_layer, n_batch, n_hidden]
