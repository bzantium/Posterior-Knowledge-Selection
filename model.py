...
class Encoder(nn.Module):
    ...
    def forward(self, X):
        ...
        packed_inputs = rnn.pack_padded_sequence(
            inputs, seq_lengths.to(torch.device("cpu")), enforce_sorted=False
        )
        packed_outputs, hidden = self.gru(
            packed_inputs
        )  # hidden: [2*n_layer, n_batch, n_hidden]
        ...
        encoded = torch.cat(
            (f_hidden, b_hidden), dim=1
        )  # encoded: [n_batch, 2*n_hidden]
    ...
    def forward(self, K):
        ...
        if len(K.shape) == 3:  # [n_batch, N, seq_len]
            ...
            for i in range(N):
                ...
                packed_inputs = rnn.pack_padded_sequence(
                    k, seq_lengths.to(torch.device("cpu")), enforce_sorted=False
                )
                _, hidden = self.gru(
                    packed_inputs
                )  # hidden: [2*n_layer, n_batch, n_hidden]
            ...
            return encoded.transpose(0, 1).to(device)  # [n_batch, N, 2*n_hidden]
...
class Manager(nn.Module):
    ...
    def forward(self, x, y, K):
        ...
        if y is not None:
            ...
            k_idx = gumbel_softmax(
                posterior_logits, self.temperature
            )  # k_idx: [n_batch, N(one_hot)]
            k_i = torch.bmm(K, k_idx.unsqueeze(2)).squeeze(
                2
            )  # k_i: [n_batch, 2*n_hidden]
            k_logits = F.log_softmax(
                self.mlp_k(k_i), dim=-1
            ...
            return (
                ...
                k_i,
                k_logits,
            )  # prior: [n_batch, N], posterior: [n_batch, N]
        else:
            n_batch = K.size(0)
            k_i = torch.Tensor(n_batch, 2 * self.n_hidden).to(device)
...
class Attention(nn.Module):
    ...
    def forward(
        self, hidden, encoder_outputs, encoder_mask=None
    ):  # hidden: [n_batch, n_hidden]
        seq_len = encoder_outputs.size(
            0
        )  # encoder_outputs: [seq_len, n_batch, n_hidden]
        h = hidden.repeat(seq_len, 1, 1).transpose(0, 1)  # [n_batch, seq_len, n_hidden]
        encoder_outputs = encoder_outputs.transpose(
            0, 1
        )  # [n_batch, seq_len, n_hidden]
        attn_weights = self.score(
            h, encoder_outputs, encoder_mask
        )  # [n_batch, 1, seq_len]
        return attn_weights
...
class Decoder(nn.Module):  # Hierarchical Gated Fusion Unit
    ...
    def forward(self, input, k, hidden, encoder_outputs, encoder_mask=None):
        ...
        context = torch.bmm(
            attn_weights, encoder_outputs.transpose(0, 1)
        )  # [n_batch, 1, n_hidden]
        ...
        _, y_hidden = self.y_gru(
            y_input, hidden
        )  # y_hidden: [n_layer, n_batch, n_hidden]
        _, k_hidden = self.k_gru(
            k_input, hidden
        )  # k_hidden: [n_layer, n_batch, n_hidden]
        t_hidden = torch.tanh(
            torch.cat((self.y_weight(y_hidden), self.k_weight(k_hidden)), dim=-1)
        )
        # t_hidden: [n_layer, n_batch, 2*n_hidden]
        r = torch.sigmoid(self.z_weight(t_hidden))  # [n_layer, n_batch, n_hidden]
        hidden = torch.mul(r, y_hidden) + torch.mul(
            1 - r, k_hidden
